...
public class SpinTest {
    ...
    static boolean isDealerRaised = false, isSBRaised = false, isBBRaised  =false;
    static boolean isSBFolds = false, isDealerFolds=false, isBBFolds=false;
    public static boolean gameOver = false, split = false;

    public static int heroWinCount = 0, foldsCount = 0;
    ...
    public static void Round ()
    {
        ...
        while (!gameOver) {
            SpinTest.pot = 0;
            SpinTest.lastStake=0;
            SpinTest.foldsCount = 0;
            Player[] plArr;
            int pocketWeight = 0;
            isDealerAllIn = false;isSbAllIn = false;isBBAllIn = false;
            isDealerRaised = false; isSBRaised = false; isBBRaised  =false;
    ...
    {
        int pocketWeight = Main.player1.getPocket().get(0).getRank() + Main.player1.getPocket().get(1).getRank();
        switch (p.getPosition())
        {
            case 0:
                if (lastStake == 0) {
                    if ((pocketWeight >= 12) ||
                            ((p.getPocket().get(0).getSuit() == p.getPocket().get(1).getSuit()) && (pocketWeight >= 10)) ||
                            (p.getPocket().get(0).getRank() == p.getPocket().get(0).getRank())) {
                        SpinTest.pot += bb * 2;
                        lastStake = bb * 2;
                        p.setStack(-bb*2);
                        isDealerRaised = true;
                        System.out.println("Dealer raise first");
                ...
                {
                    if ((pocketWeight >= 20) ||
                            ((p.getPocket().get(0).getSuit() == p.getPocket().get(1).getSuit()) && (pocketWeight >= 18)) ||
                            ((p.getPocket().get(0).getRank() == p.getPocket().get(0).getRank())&& (pocketWeight >= 12)))
                    {
                        pot+=p.getStack();
                        lastStake = p.getStack();
                        p.setStackDirect(0);
                        isDealerAllIn = true;
                        System.out.println("Dealer ALLIN");
            ...
            case 1:
                if (lastStake == 0) {
                    if ((pocketWeight >= 10) ||
                            ((p.getPocket().get(0).getSuit() == p.getPocket().get(1).getSuit()) && (pocketWeight >= 6)) ||
                            (p.getPocket().get(0).getRank() == p.getPocket().get(0).getRank())) {
                        SpinTest.pot += bb * 2;
                        lastStake = bb * 2;
                        p.setStack(-bb*2);
                        isSBRaised = true;
                        System.out.println("SB raised first");

                    }
                    else
                    {
                        isSBFolds = true;
                        System.out.println("SB Folds");
                    }
                }
                else
                {
